...
class FitnessWorkflow:
    ...
    async def _manage_profile(self, state: FitnessState) -> FitnessState:
        ...
        try:
            ...
            state["messages"].append(
                SystemMessage(
                    content=f"User profile updated with target: {updated_profile.target_calories} calories"
                )
            )
    ...
    async def _plan_workout(self, state: FitnessState) -> FitnessState:
        ...
        try:
            ...
            state["messages"].append(
                SystemMessage(
                    content=f"Workout plan generated: {request.split_type} split"
                )
            )
    ...
    async def _coordinate_plans(self, state: FitnessState) -> FitnessState:
        ...
        try:
            ...
            state["messages"].append(
                SystemMessage(
                    content=f"Plans coordinated: {coordination_message.content}"
                )
            )
