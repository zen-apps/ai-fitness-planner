...
class MealPlannerAgent:
    ...
    async def find_foods_by_criteria(self, criteria: Dict[str, Any]) -> List[Dict]:
        ...
        try:
            ...
            if criteria.get("food_category"):
                query["foodCategory"] = {"$regex": criteria["food_category"], "$options": "i"}
    ...
    async def generate_meal_plan(self, profile: UserProfile, request: MealPlanRequest) -> Dict[str, Any]:
        ...
        prompt = ChatPromptTemplate.from_messages([
            ("system", """You are a nutrition expert creating personalized meal plans.
            ...
            """),
            ("human", "Create my personalized meal plan based on the profile above.")
        ])
...
class WorkoutPlannerAgent:
    ...
    async def generate_workout_plan(self, profile: UserProfile, request: WorkoutPlanRequest) -> Dict[str, Any]:
        """Generate a complete workout plan"""
        
        prompt = ChatPromptTemplate.from_messages([
            ("system", """You are a certified personal trainer creating workout plans.
            ...
            - summary: brief explanation of plan approach
            """),
            ("human", "Create my personalized workout plan based on the profile above.")
        ])
        ...
        try:
            ...
            response = await chain.ainvoke({
                "fitness_goal": profile.fitness_goal,
                "equipment_available": profile.equipment_available or ["bodyweight", "dumbbells", "barbell"],
...
class PlanSummaryAgent:
    ...
    async def create_summary(self, profile: UserProfile, meal_plan: Dict, workout_plan: Dict) -> str:
        """Create a comprehensive plan summary"""
        
        prompt = ChatPromptTemplate.from_messages([
            ("system", """You are a fitness coach providing comprehensive guidance.
...
async def generate_complete_fitness_plan(
    ...
    try:
        ...
        workout_plan = await workout_agent.generate_workout_plan(profile, workout_request)
        ...
        raise HTTPException(status_code=500, detail=f"Failed to generate complete plan: {str(e)}")
