...
class FitnessAPI:
    ...
    def create_profile(profile_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            response = requests.post(
                f"{API_BASE_URL}/v1/agents/profile/",
                json=profile_data,
                timeout=30
            )
    ...
    def get_profile(user_id: str) -> Dict[str, Any]:
        ...
        try:
            response = requests.get(
                f"{API_BASE_URL}/v1/agents/profile/{user_id}",
                timeout=30
            )
    ...
    @staticmethod
    def generate_complete_plan(user_id: str, meal_days: int = 7, workout_days: int = 3) -> Dict[str, Any]:
        """Generate complete fitness plan"""
        try:
            params = {
                "user_id": user_id,
                "meal_request": {"user_id": user_id, "days": meal_days, "meal_count": 3},
                "workout_request": {"user_id": user_id, "days_per_week": workout_days}
            }
            
            response = requests.post(
                f"{API_BASE_URL}/v1/agents/complete-plan/",
                json=params,
                timeout=60
            )
            response.raise_for_status()
...
def main():
    ...
    with st.sidebar:
        ...
        page = st.selectbox(
            "Choose a page:",
            ["🏠 Home", "👤 Profile Setup", "🍽️ Meal Plans", "💪 Workout Plans", "📊 Complete Plan", "🔍 Food Search"]
        )
...
def show_profile_page():
    ...
    with st.form("profile_form"):
        ...
        equipment_available = st.multiselect(
            "Available Equipment",
            ["bodyweight", "dumbbells", "barbell", "resistance_bands", "pull_up_bar", "gym_access"],
            default=existing_profile.get("equipment_available", ["bodyweight", "dumbbells"])
        )
        ...
        if submitted:
            profile_data = {
                ...
                "created_at": datetime.now().isoformat() if not existing_profile else existing_profile.get("created_at")
            }
...
def show_meal_plans_page():
    ...
    if not st.session_state.get("profile_created") and not st.session_state.get("current_profile"):
    ...
    if st.button("🍽️ Generate Meal Plan", use_container_width=True):
        with st.spinner("Creating your personalized meal plan..."):
            ...
            st.info("🚧 Meal plan generation coming soon! Use the Complete Plan page for now.")
...
def show_workout_plans_page():
    ...
    st.markdown("Generate personalized workout plans based on your goals and available equipment.")
    ...
    with col2:
        ...
        session_duration = st.selectbox("Session Duration (min)", [30, 45, 60, 75, 90], index=2)
    ...
    if st.button("💪 Generate Workout Plan", use_container_width=True):
        with st.spinner("Creating your personalized workout plan..."):
            ...
            st.info("🚧 Workout plan generation coming soon! Use the Complete Plan page for now.")

def show_complete_plan_page():
    """Complete plan generation page"""
    st.header("📊 Complete Fitness Plan")
    
    if not st.session_state.get("current_profile") and not FitnessAPI.get_profile(st.session_state.user_id):
        st.warning("⚠️ Please create your profile first!")
        if st.button("👤 Go to Profile Setup"):
            st.rerun()
        return
    
    st.markdown("Generate your complete personalized fitness plan including both meal and workout plans!")
    ...
    if st.button("🔍 Search Foods", use_container_width=True) and search_query:
        with st.spinner("Searching nutrition database..."):
            ...
            if results and results.get("results"):
                st.success(f"Found {results['results_found']} results for '{search_query}'")
                
                for i, food in enumerate(results["results"]):
                    with st.expander(f"🥘 {food.get('description', 'Unknown Food')} - {food.get('brand_owner', 'Unknown Brand')}"):
                        ...
                        with col1:
                            ...
                            st.write(f"**Serving Size:** {food.get('serving_size', 'N/A')} {food.get('serving_size_unit', '')}")
                            
                            if food.get("ingredients"):
                                st.write(f"**Ingredients:** {food['ingredients'][:200]}{'...' if len(food['ingredients']) > 200 else ''}")
                        ...
                        with col2:
                            ...
                            if per_100g:
                                ...
                                if macro_breakdown.get("primary_macro_category"):
                                    st.write(f"**Primary Macro:** {macro_breakdown['primary_macro_category'].replace('_', ' ').title()}")
                            else:
