...
class MessagingAgent(Agent):
    ...
    def __init__(self):
        ...
        if DO_TEXT:
            ...
            self.me_from = os.getenv(
                "TWILIO_FROM", "your-phone-number-if-not-using-env"
            )
            self.me_to = os.getenv(
                "MY_PHONE_NUMBER", "your-phone-number-if-not-using-env"
            )
    ...
    def message(self, text):
        ...
        message = self.client.messages.create(
            from_=self.me_from, body=text, to=self.me_to
        )
